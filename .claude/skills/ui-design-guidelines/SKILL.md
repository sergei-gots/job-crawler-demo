...
- **Application pages should not use centered layouts.** Page content is left-positioned with
  ...
  pages. See [`/.claude/features/03_FEATURE_CRAWL_SEARCH_SEPARATION.md`](/.claude/features/03_FEATURE_CRAWL_SEARCH_SEPARATION.md)'s
  Phase 3b decisions.
...
- **Password fields always use [`shared/ui/password-input.tsx`](/apps/web/shared/ui/password-input.tsx)**
  (`PasswordInput`), never a bare
...
- **Use design tokens, not hardcoded colors.** Prefer `text-muted-foreground`, `border-border`,
  `text-foreground` etc. (defined in [`app/globals.css`](/apps/web/app/globals.css)) over
  hardcoded Tailwind colors like
...
- **Typography hierarchy** — three levels, distinguished by size *and* weight together (not just a
  ...
  | --- | --- | --- | --- |
  | Page title | [`shared/ui/page-title.tsx`](/apps/web/shared/ui/page-title.tsx)'s `PageTitle` (one `<h1>` per page) | `text-2xl font-semibold tracking-tight` | 24px / 600 |
  | Section heading | [`shared/ui/card.tsx`](/apps/web/shared/ui/card.tsx)'s `CardTitle` | `text-lg font-semibold` (`text-base font-semibold` in `size="sm"` cards) | 18px / 600 (16px / 600) |
  | Form label | [`shared/ui/label.tsx`](/apps/web/shared/ui/label.tsx)'s `Label` | `text-sm font-medium` | 14px / 500 |
...
- **Button color hierarchy** — two levels, distinguished by fill, not by inventing new variants:
  ...
  | --- | --- | --- | --- |
  | Primary action | `variant="default"` ([`shared/ui/button.tsx`](/apps/web/shared/ui/button.tsx)) | Medium-dark gray fill (`--primary: oklch(0.55 0 0)`), light text — deliberately *not* near-black | Main CTA per screen: Login, Register, Update profile |
  ...
  Both colors are tokens in [`app/globals.css`](/apps/web/app/globals.css) (`:root` block, light theme) — tune brightness there,
...
- **External/navigational links use the `text-link` utility, not a raw color or bare `<a>`.**
  ...
  ([`widgets/sources/ui/sources-page.tsx`](/apps/web/widgets/sources/ui/sources-page.tsx)).
- **User-facing text uses a plain hyphen (`-`), not an en dash (`–`) or em dash (`—`).** The
  ...
  `apps/web` UI strings (labels, placeholders, copy) — not to docs ([`CLAUDE.md`](/CLAUDE.md),
  [`README.md`](/README.md), [`ARCHITECTURE.md`](/ARCHITECTURE.md)) or code comments, where an em
  dash is fine.
- **`CrawlLog` lines are colored by `level`, not left uniformly `text-foreground`.** `ERROR`-level
  ...
  detail page's Execution logs panel
  ([`widgets/source-detail/ui/source-detail-page.tsx`](/apps/web/widgets/source-detail/ui/source-detail-page.tsx)). Extend
...
- **Data-wiping admin actions ("Clear cache", "Clear search data", "Clear data") use
  ...
  *entire* shared Redis cache or Elasticsearch corpus, or one source's full crawl history, not
  ...
  is what actually gates the action. `--destructive` itself ([`app/globals.css`](/apps/web/app/globals.css)) was darkened from
  ...
  a second red.
- **`StatusBadge` ([`shared/ui/status-badge.tsx`](/apps/web/shared/ui/status-badge.tsx)) appends a status icon, not just color.**
