...
written (or updated) as part of that work — not after. These docs are the durable record of
**why**: the code shows *what* was built, the doc captures the reasoning, the alternatives
rejected, and the decisions locked with the user so they aren't silently re-litigated later.
...
- **Naming**: `<NN>[letter]_FEATURE_<SHORT_NAME>.md`, uppercase snake case, prefixed with the
  ...
  [`02_FEATURE_REAL_CRAWLER_REDIS_ES.md`](/.claude/features/02_FEATURE_REAL_CRAWLER_REDIS_ES.md)
  for Increment 2,
  [`02b_FEATURE_VACANCY_DETAIL_CRAWL.md`](/.claude/features/02b_FEATURE_VACANCY_DETAIL_CRAWL.md)
  for the Increment 2.2 sub-increment that follows it,
  [`03_FEATURE_CRAWL_SEARCH_SEPARATION.md`](/.claude/features/03_FEATURE_CRAWL_SEARCH_SEPARATION.md)
  for Increment 3). Use a trailing lowercase letter (`b`, `c`, ...) for a `.N` sub-increment rather
...
- **Keep it in sync**: when the work lands, update the doc's Status and check off its steps; when a
  ...
  for [`/CLAUDE.md`](/CLAUDE.md)/[`/README.md`](/README.md)/[`/ARCHITECTURE.md`](/ARCHITECTURE.md)
  drift.
